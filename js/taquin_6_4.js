...
function start() {
  ...
  for (i = 0; i < 4; i++) for (j = 0; j < 6; j++) {
    ...
    c.col = j;
    c.row = i;
    c.style.width = c.style.height = '168px';
    offsetx = (c.id%6)*(-168)+'px';
    offsety = Math.floor(c.id/6)*(-168)+'px';
    c.style.backgroundPosition = offsetx+' '+offsety;
    if (i != 0 || j != 0) tiles[taquin[i][j]].moveTo(150+j*168, 50+i*168, 1);
  }
  taquin[0][0] = '';
  cnt = 0;
...
function tileAction(id) {
  ...
  t.moveTo(150+t.col*168, 50+t.row*168, 1);
}
